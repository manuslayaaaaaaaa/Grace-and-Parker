...
(function () {
  const TAP_MAX_MOVE_PX = 12;   // iets ruimer voor mobiel
  const TAP_MAX_TIME_MS = 400;

  let startX = 0, startY = 0, startTime = 0;
  let moved = false;

  // Pointerdown: start meten alleen als we op de product image zitten
  document.addEventListener('pointerdown', function (e) {
    const img = e.target.closest('img.photoswipe__image');
    if (!img) return;
  ...
  // Pointermove: als gebruiker swipet/dragged => moved = true
  document.addEventListener('pointermove', function (e) {
    if (!startTime) return; // alleen als we gestart zijn
    const dx = Math.abs(e.clientX - startX);
    const dy = Math.abs(e.clientY - startY);
    if (dx > TAP_MAX_MOVE_PX || dy > TAP_MAX_MOVE_PX) moved = true;
  }, true);

  // Pointerup: als het een tap was => trigger zoom button binnen dezelfde slide
  document.addEventListener('pointerup', function (e) {
    ...
    // Swipe/drag? Dan niets doen, Flickity mag zijn werk doen.
    if (moved) return;
    if (elapsed > TAP_MAX_TIME_MS) return;

    // Zoek de huidige slide/container en klik de echte zoom-knop
    const slide =
      img.closest('.product-main-slide') ||
      img.closest('[data-product-image-main]') ||
      img.closest('.image-wrap') ||
      document;

    const zoomBtn = slide.querySelector('button.js-photoswipe__zoom');
    if (!zoomBtn) return;

    // Dit is een tap => zoom openen
    e.preventDefault();
    e.stopPropagation();
    zoomBtn.click();
  }, true);
